...
public class NovinEndpointDocumentFilter : IDocumentFilter
{
                ...
                foreach (var parameter in methodDefinition.Parameters)
                    ...
                    else if (parameter.In is InQuery)
                    {
                        if (parameter.Type?.GetInterface("IQuery") is not null)
                            ...
                            MapRequestGetParameter(parameter, operation);
                    }
    ...
    private static void MapRequestGetParameter(EndpointDocumentParameter parameter, OpenApiOperation operation)
    {
        foreach (var property in parameter.Type?.GetProperties()!)
        {
            ...
            if (fromHeaderAttribute != null)
            {
                ...
                name = fromHeaderAttribute?.Name ?? name;
            }
            else if (fromRouteAttribute != null)
            {
                location = ParameterLocation.Path;
                name = fromRouteAttribute?.Name ?? name;
            }
            else 
            {
                location = ParameterLocation.Query;
                name = fromQueryAttribute?.Name ?? name;
            }
